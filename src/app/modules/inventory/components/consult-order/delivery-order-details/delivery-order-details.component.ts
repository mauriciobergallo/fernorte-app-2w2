...
export class DeliveryOrderDetailsComponent {
  ...
  constructor(private route: ActivatedRoute, private deliveryorderService: DeliverOrderService, private router: Router) 
  {
  }
  ...
  save() {
    ...
    this.deliveryorderService.updateDeliveryOrderDetails(deliveryOrderPut)
      .pipe(
        switchMap(() => {
  ...
  ngOnInit() {
    this.route.paramMap.subscribe(params => {
      ...
      if (id !== null) {
        this.orderId = +id;
        this.deliveryorderService.getDeliveryOrderById(this.orderId).subscribe(orderData => {
          this.order = orderData;
        });
      }
    });
