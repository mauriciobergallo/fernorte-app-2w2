...
export class DeliveryOrdersMockService {
  ...
  filterByString(filterstr : string)
  {
    ...
    {
      var filteredList = this.listFilteredDate.filter(item =>
        (item.client?.first_name + ' ' + item.client?.last_name)
              .toLowerCase()
              .includes(filterstr.trim().toLowerCase()) ||
        item.client?.document_number.toLowerCase().includes(filterstr.toLowerCase())
      );
    }else
    {
      var filteredList = this.originallist.filter(item =>
        (item.client?.first_name + ' ' + item.client?.last_name)
              .toLowerCase()
              .includes(filterstr.trim().toLowerCase()) ||
        item.client?.document_number.toLowerCase().includes(filterstr.toLowerCase())
...
getById(id: number) {
  var deliveryOrder = this.originallist.find(order => order.delivery_order_id === id);
  return deliveryOrder;
}
