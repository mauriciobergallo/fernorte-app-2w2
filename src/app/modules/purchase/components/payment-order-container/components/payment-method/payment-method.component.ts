...
export class PaymentMethodComponent implements OnInit {
  ...
  savePaymentOrderDetails() {
    const hasNullPaymentMethod = this.paymentOrderDetails.some(
      (detail) => detail.paymentMethod === null
    );
    if (hasNullPaymentMethod) {
      // Display an alert message
      Swal.fire(
        'Advertencia',
        'Por favor seleccione un metodo de pago para todas las ordenes de compra',
        'warning'
      );
