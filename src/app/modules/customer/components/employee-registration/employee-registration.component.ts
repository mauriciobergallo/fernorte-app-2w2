...
export class EmployeeRegistrationComponent implements OnInit {
	...
	open(content: any) {
		...
		this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title', backdrop: 'static' }).result.then(
			(result) => {
				...
				const formattedBirthDate = new Date(ngbDate.year, ngbDate.month - 1, ngbDate.day).toISOString();

				let newEmployee: any = {
					first_name: this.employeeForm.value.firstName,
					last_name: this.employeeForm.value.lastName,
					birth_date: formattedBirthDate,
					document_type: this.employeeForm.value.documentType,
					address: this.employeeForm.value.address,
					phone_number: this.employeeForm.value.phoneNumber,
					document_number: this.employeeForm.value.documentNumber,
					personal_email: this.employeeForm.value.personalEmail,
				  };

				  let employeeInSnake = this.conversion.toSnakeCase(newEmployee);

				  this.employeeService.postEmployee(employeeInSnake).subscribe(
					(response) => {
	...
	onSubmitForm() {
		if (this.employeeForm.valid) {
			...
			const employeeData = this.conversion.toSnakeCase(this.employeeForm.value);
			...
			this.employeeService.postEmployee(employeeInSnake).subscribe(
				(newEmployee: EmployeeResponseDTO) => {
					...
					this.modalService.dismissAll(newEmployee);
	...
	showConfirmation() {
		...
		Swal.fire({
			title: '¿Estás seguro?',
			text: '¿Quieres Registrar al nuevo empleado?',
			icon: 'question',
			showCancelButton: true,
